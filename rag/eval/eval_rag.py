...
Usage:
    uv run python -m rag.eval.eval_rag                          # default hybrid config
    uv run python -m rag.eval.eval_rag --mode bm25|dense|hybrid
    uv run python -m rag.eval.eval_rag --sweep-aux              # aux vs no-aux
    uv run python -m rag.eval.eval_rag --sweep-path-boost 0,1,3,5
    uv run python -m rag.eval.eval_rag --sweep-rrf-k 20,40,60,120
    uv run python -m rag.eval.eval_rag --gold-set ext           # extended gold set
    uv run python -m rag.eval.eval_rag --verbose
"""
    ...
    if mode in ("hybrid", "dense") and vecs_path.exists():
        ...
        if count != len(rows):
            print(f"[eval] WARNING: vectors.f32 has {count} rows but the corpus "
                  ...
                  f"this ablation; rebuild with `python -m rag compile --steps index "
...
def _assert_vector_alignment(idx_dir: Path, rows, sample: int = 64) -> None:
    ...
    if len(built) != len(rows):
        raise RuntimeError(
            ...
            f"(`python -m rag compile --steps index --force`) before evaluating dense.")
...
def main() -> int:
    ...
    if not rows:
        raise SystemExit("corpus is empty — run python -m rag compile first")
...
def emit(results: list[dict], args) -> int:
    ...
    """
    path = Path(__file__).resolve().parents[2] / "eval_results_latest.md"
    lines = [
        "# Retrieval evaluation results (latest raw run)",
        "",
        "Auto-generated by rag/eval/eval_rag.py — raw rows only. The curated findings,"
        "ablations and caveats live in eval_results.md; copy rows across by hand.",
        "",
        f"Run: `python -m rag.eval.eval_rag {' '.join(a for a in sys.argv[1:])}`",
        "",
    ...
    n_html = count_html_pages(cfg.get("dirs", []), Path(cfg.get("_base_dir", ".")))
