...
from rag.llm.base import RETRY_DELAYS, fmt_delay, retry_schedule
...
async def run_corpus_step(
    ...
    *,
    ...
    # Transient-failure backoff schedule for every LLM call (429 / 5xx / timeout /
    # connection reset). Validated ONCE here — before a page is even planned — so
    # a bad "retry_delays" fails fast with a clear message instead of per page.
    try:
        retry_delays = retry_schedule(cfg.get("retry_delays"))
    except (ValueError, TypeError) as exc:
        raise SystemExit(
            f'config "retry_delays" {exc} — expected a list of seconds, e.g. '
            f'{list(RETRY_DELAYS)}') from exc
    print(f"[corpus] retry schedule: "
          f"{' -> '.join(fmt_delay(d) for d in retry_delays)}"
          f"{' (default)' if cfg.get('retry_delays') is None else ''}",
          file=sys.stderr)
    ...
    # Planning is driven by the md5 diff and the filesystem ONLY: model
    # switches, prompt/extractor/schema version bumps and needs_regen flags
    # do not requeue pages (see plan_work). Switching the LLM mid-build is
    # expected operating practice, not a reason to start over.
    work = plan_work(fm, store, diff, force=force,
